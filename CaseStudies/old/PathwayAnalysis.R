...
# load ALS data
DE_data_ALS <- read.csv("ALS/topTable_ALS.gray.matter.Sporadic - Control.gray.matter.csv")

# Annotate probes with HGNC symbol and ENSEMBL ID
ensembl=useMart("ensembl")
ensembl = useDataset("hsapiens_gene_ensembl",mart=ensembl)
annotations_ALS <- getBM(attributes=c("ensembl_gene_id",
                                      "hgnc_symbol",
                                      "affy_hugenefl"), 
                         filters = "affy_hugenefl",
                         values = DE_data_ALS$GeneID,
                         mart = ensembl)

# Save annotations
save(annotations_ALS, file = "ProbeAnnotations_ALS.RData")
